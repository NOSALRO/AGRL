import numpy as np
import torch
from vae import VariationalEncoder, VAE, Decoder
...
def loss_fn(x_target, x_hat, x_hat_var, mu, log_var, beta, eps=1e-06):
    gnll = torch.nn.functional.gaussian_nll_loss(x_hat, x_target, torch.exp(x_hat_var))
...
for epoch in range(epochs):
    ...
    for i in dataset:
        ...
        _loss = loss_fn(i.float().to(device), x_hat, x_hat_var, mu, log_var, beta=1)
        _loss.backward()
