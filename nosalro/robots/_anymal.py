...
class Anymal(SingleRigidBodyDynamics):

    def __init__(self):
        inertia = self._inertia_tensor(0.946438, 1.94478, 2.01835, 0.000938112, -0.00595386, -0.00146328)
        ...
        super().__init__(anymal_mass, inertia, feet_ref_positions, feet_min_bounds, feet_max_bounds, T, T_swing)

        self._base_position = np.array([0., 0., np.abs(z_nominal_b)]).reshape((3, 1))
        self._feet_positions = feet_positions
        self._feet_phases = [T_swing, 0, 0, T_swing]

    @staticmethod
    def _inertia_tensor(Ixx, Iyy, Izz, Ixy = 0., Ixz = 0., Iyz = 0.):
